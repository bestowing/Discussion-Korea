...
final class LawManager {
    ...
    private func initialLaws() -> Map {
        ...
        lawItems.append({
            ...
            var array = [Any]()
            array.append("본 플랫폼은 상호 존중에 입각하여 토론함을 원칙으로 한다.")
            array.append("필요 이상으로 논쟁이 과열되어, 분쟁이 발생한 경우, 1차적으로 함께 토론하던 다른 위원들이 중재한다.")
            array.append("1차적 중재가 통하지 않을 시, 당사자 또는 제3자는 분쟁조정위원장에게 중재를 요청할 수 있다.")
            array.append("제 3항에서 중재요청을 받은 분쟁조정위원장은 토론에 개입하여 분쟁을 중재할 수 있다.")
            array.append({
                var map = Map()
                map["topic"] = "제 4항에서 중재를 진행한 분쟁조정위원장은 분쟁조정요청자에게 다음과 같이 결과를 통보하여야 한다."
                map["contents"] = [
                    "분쟁조정완료(토론 진행 중 해결 시)",
                    "공식적인 경고",
                    "강제 퇴장 (경고 2회 초과시)"
